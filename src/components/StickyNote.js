...
const StickyNote = (props) => {
  ...
  const [isColorCustomizerVisible, setIsColorCustomizerVisible] =
    useState(false);
  ...
  useEffect(() => {
    ...
    if (
      debouncedTitle === props.note.title &&
      debouncedContent === props.note.content
    )
      return;
  ...
  return (
    <div className="sticky-note" style={{ backgroundColor: props.note.color }}>
      ...
      {isColorCustomizerVisible && (
        <ColorCustomizer onColorChange={handleOnColorChange} />
      )}
      <>
        {!editingTitle && (
          <div
            className="sticky-note-title"
            onClick={() => !isColorCustomizerVisible && setEditingTitle(true)}
          >
            {title}
          </div>
        )}
      ...
      <>
        {!editingContent && (
          <div
            ...
            onClick={() => !isColorCustomizerVisible && setEditingContent(true)}
          >
            {content}
          </div>
        )}
